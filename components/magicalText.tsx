...
const Star = (props: StarProps) => {
    ...
    return (
        <span className={`block absolute top-[${position.top}%] left-[${position.left}%] h-6 w-6 fill-[${props.color}] opacity-70 ${isAnimated ? `animate-[scale_${props.scaleDuration}ms_ease_forwards]` : 'hidden'}`}> 
            <svg viewBox="0 0 512 512" className={`animate-[rotate_${props.rotateDuration}ms_linear_infinite]`}>
    ...
    return (
        <>
            <span className='inline-block relative'>
                {starSvgSpans}
                <span
                    className={`bg-gradient-to-r ${gradientClassName} bg-clip-text text-transparent text-inherit bg-[length:200%] animate-[background-pan_${backgroundPanDuration}ms_linear_infinite]`}
                >
                    {children}
                </span>
            </span>
